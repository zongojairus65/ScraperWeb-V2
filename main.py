...
def run_llm(content: str, prompt: str) -> tuple[str, str]:
    ...
    Modele principal : Gemini (models/gemini-flash-latest)
    Fallback         : Magistral (Mistral) si Gemini echoue ou n'est pas configure
...
Reponds de maniere precise et structuree en te basant uniquement sur le contenu ci-dessus."""
    ...
    try:
        ...
        raise RuntimeError(
            f"Echec Gemini ({gemini_error}) et echec Magistral ({fallback_error})"
        )
...
#           Gemini (gemini-flash-latest), fallback Magistral, pour l'analyse finale
# ─────────────────────────────────────────────────────────────────────────────
...
async def root():
    return {
        "message": "ScraperWeb V2 — Powered by Gemini + Magistral (fallback)",
        "endpoints": {
...
async def health_check():
    return {"status": "healthy", "service": "ScraperWeb V2", "primary_model": GEMINI_MODEL, "fallback_model": MAGISTRAL_MODEL}
...
async def agent_search(request: AgentSearchRequest):
    ...
    Gemini (avec fallback Magistral) analyse ensuite le contenu extrait et repond a la question.
    ...
    try:
        ...
        # Avertissement si la derniere etape avant extraction etait en echec/forcee
        # (le contenu extrait risque alors de ne pas correspondre a l'objectif).
        warning = None
        steps = agent_result["steps"]
        if len(steps) >= 2:
            prev_step = steps[-2]
            if prev_step.get("error") or steps[-1].get("note"):
                warning = (
